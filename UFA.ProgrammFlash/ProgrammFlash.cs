...
namespace UFA.ProgrammFlash
{
    ...
    public class ProgrammFlash
    {
        ...
        private void AddressChecker(I32HEX hexLine, TypeFRM type, ref ushort pageStot)
        {
            if ((hexLine.RecordType == (byte)RecordType.Data) && ((hexLine.Address % 0x8000) == 0))
            {
                if (type == TypeFRM.DSP)
                    pageStot++;
                else
        ...
        virtual public void IntelHexCheckFile(FileInfo FRM, TypeFRM type)
        {
            using (StreamReader reader = new StreamReader(FRM.FullName))
            {
                try
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        AddressChecker(hex, type, ref MaxPage);
                    }
                    if (type == TypeFRM.DSP)
                        this.PagesZone.DSP.Maximum = (ushort)(MaxPage + this.PagesZone.DSP.Minimum);
                    else
                        this.PagesZone.PLIS.Maximum = (ushort)(MaxPage + this.PagesZone.PLIS.Minimum);
